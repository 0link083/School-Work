...
public:
	...
	static Employee* fromXML(istream& f) {
		if (f) {
			...
			int i = 0; // Just to see how many lines are read
			...
			while (getline(f, line)) {
				...
				if (line == "<EMPLOYEE>" && empFlag == false) { //Line will probably just be EMPLOYEE since it will delemit by <>
					try {
						...
						cerr << e.what() << endl;
						//Since they want a graceful exit, we should probably end the tag for them
					}
				}
					...
					try {
						...
						// We leave after creating an object
					}
					catch (runtime_error &e) {
						cerr << e.what() << endl;
				...
				else if (empFlag) { // If we can assume another tag, we should. Otherwise we lose it.
					// <Salary> </Salary>
